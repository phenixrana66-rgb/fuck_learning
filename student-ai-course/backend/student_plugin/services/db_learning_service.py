...
def _find_section(db: Session, lesson_db_id: int, section_identifier: str | int | None) -> LessonSection | None:
    ...
    query = (
        ...
        .options(
            joinedload(LessonSection.pages),
            joinedload(LessonSection.anchors),
            joinedload(LessonSection.knowledge_points),
            joinedload(LessonSection.unit),
        )
        .filter(LessonSection.lesson_id == lesson_db_id)
    )
...
def _get_section_progress_map(db: Session, student_db_id: int | None, lesson_db_id: int) -> dict[int, StudentSectionProgress]:
    ...
    rows = (
        ...
        .filter(StudentSectionProgress.student_id == student_db_id, StudentSectionProgress.lesson_id == lesson_db_id)
        .all()
    )
    return {row.section_id: row for row in rows}


def _find_latest_practice_attempt(
    db: Session,
    student_db_id: int | None,
    lesson_db_id: int,
    section_db_id: int,
) -> StudentPracticeAttempt | None:
    if not student_db_id:
        return None
    return (
        ...
        .filter(
            ...
            StudentPracticeAttempt.lesson_id == lesson_db_id,
            StudentPracticeAttempt.section_id == section_db_id,
            StudentPracticeAttempt.grading_status == "graded",
        )
        .order_by(StudentPracticeAttempt.submitted_at.desc(), StudentPracticeAttempt.id.desc())
        .first()
    )
...
def _build_db_chapter(section: LessonSection, unit_title: str, progress: StudentSectionProgress | None, fallback_page_no: int) -> dict:
    knowledge_points = [
        point.point_name for point in sorted(section.knowledge_points or [], key=lambda item: (item.sort_no, item.id))
    ]
    return {
...
def enhance_player_with_db(db: Session, player: dict, student_id: str | int | None) -> dict:
    ...
    if chapters:
        player_copy["progressPercent"] = _round_int(
            sum(int(chapter.get("progressPercent") or 0) for chapter in chapters) / len(chapters)
        )
        player_copy["masteryPercent"] = _round_int(
            sum(int(chapter.get("masteryPercent") or 0) for chapter in chapters) / len(chapters)
        )
...
def get_db_progress_state(db: Session, student_id: str | int | None, lesson_identifier: str | int | None) -> dict | None:
    ...
    return {
        "sectionId": str(lesson_progress.current_section_id) if lesson_progress.current_section_id else "",
        "anchorId": str(anchor.id) if anchor else "",
        "anchorTitle": anchor.anchor_title if anchor else "",
        "pageNo": lesson_progress.last_page_no or (anchor.page_no if anchor else 1) or 1,
...
def get_section_detail(
    ...
    page_progress_rows: dict[int, StudentPageProgress] = {}
    if student_db_id:
        rows = (
            db.query(StudentPageProgress)
            .filter(StudentPageProgress.student_id == student_db_id, StudentPageProgress.section_id == section.id)
            .all()
        )
        page_progress_rows = {row.lesson_page_id: row for row in rows}

    anchors_by_page: dict[int, LessonSectionAnchor] = {}
        ...
        pages.append(
            {
                ...
                "pageNo": page.page_no,
                "pageTitle": page.page_title or f"第 {page.page_no} 页",
                "pageSummary": page.page_summary or "",
                "imageUrl": _normalize_asset_url(page.ppt_page_url),
                "parsedContent": page.parsed_content or page.page_summary or "",
                "anchorId": str(anchor.id) if anchor else "",
                "anchorTitle": anchor.anchor_title if anchor else "",
                "isRead": bool(row and row.is_completed),
            }
        )
    ...
    return {
        ...
        "aiGuideContent": (
            (parse_result.normalized_content if parse_result else None)
            or (parse_result.chapter_summary if parse_result else None)
            or section.section_summary
            or ""
        ),
        "knowledgePoints": [
            point.point_name for point in sorted(section.knowledge_points or [], key=lambda item: (item.sort_no, item.id))
...
def mark_page_read(
    ...
    page = (
        ...
        .filter(LessonSectionPage.id == int(lesson_page_id), LessonSectionPage.section_id == section.id)
        .first()
    )
    if not page:
        return None
    ...
    progress_row = (
        db.query(StudentPageProgress)
        .filter(StudentPageProgress.student_id == student_db_id, StudentPageProgress.lesson_page_id == page.id)
        .first()
    )
    if not progress_row:
        progress_row = StudentPageProgress(
            student_id=student_db_id,
            course_id=lesson.course_id,
            lesson_id=lesson.id,
            section_id=section.id,
    ...
    completed_pages = (
        db.query(StudentPageProgress)
        .filter(
            StudentPageProgress.student_id == student_db_id,
            StudentPageProgress.section_id == section.id,
            StudentPageProgress.is_completed == True,  # noqa: E712
        )
        .count()
    )
    progress_percent = _round_int((completed_pages / total_pages) * 100) if total_pages else 0
    practice_attempt = _find_latest_practice_attempt(db, student_db_id, lesson.id, section.id)
    practice_percent = _round_int(
        practice_attempt.accuracy_percent if practice_attempt and practice_attempt.accuracy_percent is not None else 0
    )
    ...
    if all_section_rows:
        total_progress = _round_int(
            sum(_round_int(row.progress_percent) for row in all_section_rows) / len(all_section_rows)
        )
        overall_mastery = _round_int(
            sum(_round_int(row.mastery_percent) for row in all_section_rows) / len(all_section_rows)
        )
    else:
    ...
    db.add(
        StudentSectionMasteryLog(
            ...
            lesson_id=lesson.id,
            section_id=section.id,
            practice_attempt_id=practice_attempt.id if practice_attempt else None,
            source_type="progress_sync",
...
def interact_with_section_context(
    db: Session,
    lesson_identifier: str | int | None,
    section_identifier: str | int | None,
    question: str,
    ...
    if page_no is not None:
        current_page = next((item for item in pages if item.page_no == page_no), None)
    text = question or ""
    ...
    if current_page:
        page_label = f"第 {current_page.page_no} 页"
        page_text = current_page.parsed_content or section.section_summary or ""
        answer = (
            f"当前学习章节是《{section.section_name}》，你现在看到的是{page_label}。"
            f"建议先围绕 {', '.join(related_points[:2]) or '本章核心知识点'} 理解本页内容。"
            f"{page_text}"
        )
    else:
        chapter_text = section.section_summary or ""
        answer = (
            f"当前学习章节是《{section.section_name}》。"
            f"这次回答将围绕整章内容展开，建议重点关注 {', '.join(related_points[:3]) or '本章核心知识点'}。"
            f"{chapter_text}"
        )
    ...
    return {
        ...
        "resumeAnchor": {
            ...
            "pageNo": current_page.page_no if current_page else (pages[0].page_no if pages else 1),
        },
        "weakPoints": related_points[:2] if understanding != "complete" else [],
    }
