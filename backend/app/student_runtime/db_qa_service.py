...
def get_qa_sessions(db: Session, student_identifier, lesson_identifier):
    ...
    for session in sessions:
        ...
        result.append({"sessionId": session.session_no, "title": session.session_title or _create_session_title(first_question), "messages": payload_messages, "createdAt": _timestamp_ms(session.created_at), "updatedAt": _timestamp_ms(session.updated_at)})
    return result
...
def save_qa_session(db: Session, student_identifier, lesson_identifier, section_identifier, session_payload):
    ...
    if not section:
        section = next((item for unit in sorted(lesson.units or [], key=lambda row: (row.sort_no, row.id)) for item in sorted(unit.sections or [], key=lambda row: (row.sort_no, row.id))), None)
    if not section:
        return None
    ...
    now = datetime.now()
    session = db.query(QASession).filter(QASession.session_no == session_no, QASession.student_id == student_db_id, QASession.lesson_id == lesson.id).first()
    if not session:
        session = QASession(session_no=session_no, student_id=student_db_id, course_id=lesson.course_id, lesson_id=lesson.id, current_section_id=section.id, created_at=_datetime_from_maybe_ms(session_payload.get("createdAt")))
        db.add(session)
        db.flush()
    else:
    ...
    for item in messages_payload:
        ...
        message = QAMessage(session_id=session.id, lesson_id=lesson.id, section_id=section.id, role=role, question_type=(item.get("questionType") or "text") if role == "user" else None, message_content=item.get("content") or "", created_at=created_at)
        db.add(message)
        db.flush()
        if role == "user":
        ...
        answer = QAAnswer(answer_no=f"ans-{uuid4().hex[:16]}", session_id=session.id, question_message_id=pending_question.id, assistant_message_id=message.id, related_section_id=section.id, answer_type="text", understanding_level=understanding_level, recommended_section_id=section.id if item.get("resumePageNo") else None, recommended_page_no=item.get("resumePageNo"), recommended_anchor_id=anchor.id if anchor else None, next_sections_json=None, suggestions_json=None, created_at=created_at)
        db.add(answer)
        db.flush()
