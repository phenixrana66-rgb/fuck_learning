...
class LearningService:
    ...
    def interact(self, lesson_id, question, anchor_id, page_no):
        ...
        if any(keyword in question for keyword in ["不会", "不懂", "没明白"]):
            understanding = "weak"
        ...
        answer = f"当前章节是《{slide.get('title', '当前知识点')}》。建议你先掌握 {', '.join(related[:2]) or '本章重点'}，再结合课件内容理解它的典型应用。本章的核心说明是：{slide.get('summary', '本章节围绕核心概念、方法和例题展开。')}"
        return {"answer": answer, "relatedKnowledgePoints": related, "understandingLevel": understanding, "understandingLabel": UNDERSTANDING_LABELS[understanding], "resumeAnchor": resume_anchor, "weakPoints": related[:2] if understanding != "complete" else []}
    ...
    def resume(self, lesson_id, anchor_id):
        ...
        return {"audioUrl": lesson.get("audioUrl", ""), "resumeTime": anchor.get("startTime", 0), "anchorId": anchor.get("anchorId"), "anchorTitle": anchor.get("anchorTitle"), "pageNo": anchor.get("pageNo")}
