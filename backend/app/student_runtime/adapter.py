...
from backend.app.student_runtime.mock_data import MOCK_PLAYERS, get_default_progress, get_notifications, get_player, get_student
...
class ChaoxingAdapter:
    ...
    def _serialize_lesson(self, lesson_id):
        ...
        return {"lessonId": player["lessonId"], "courseId": player["courseId"], "courseName": player["courseName"], "lessonName": player["lessonName"], "teacherName": player["teacherName"], "category": player["category"], "status": "completed" if player["progressPercent"] >= 100 else "inProgress", "progressPercent": player["progressPercent"], "masteryPercent": 0, "coverImage": player.get("coverImage", ""), "currentPage": player.get("currentPage", 1), "currentKnowledgePointName": player.get("currentKnowledgePointName", ""), "currentChapter": player.get("currentKnowledgePointName", ""), "questionCount": player.get("questionCount", 0), "lastStudyAt": player.get("lastStudyAt", "2026-03-29 10:00"), "units": deepcopy(player.get("units", []))}

    def verify(self, token):
        if token not in {"student_demo_token_001", "test_student_token_001"}:
            raise PermissionError("invalid token")
        return {"student": deepcopy(self._student), "lessons": self.get_student_lessons(self._student["studentId"])}
    ...
    def list_notifications(self, student_id):
        return [{"id": item["id"], "title": item["title"], "summary": item["summary"], "createdAt": item["createdAt"], "type": item["type"], "read": item["read"]} for item in deepcopy(self._notifications)]
