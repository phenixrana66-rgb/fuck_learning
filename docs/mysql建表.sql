...
DROP TABLE IF EXISTS `chapter_section_audio_assets`;
...
CREATE TABLE `chapter_section_audio_assets` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='section 绾ч煶棰戣祫婧?';
...
CREATE TABLE `lesson_sections` (
  ...
  `section_audio_asset_id` BIGINT UNSIGNED DEFAULT NULL,
  `section_code` VARCHAR(64) NOT NULL,
  `section_name` VARCHAR(255) NOT NULL,
  ...
  CONSTRAINT `fk_lesson_sections_section_audio` FOREIGN KEY (`section_audio_asset_id`) REFERENCES `chapter_section_audio_assets` (`id`)
...
CREATE TABLE `student_section_progress` (
  ...
  `understanding_level` ENUM('none', 'partial', 'full') DEFAULT NULL,
  `pace_mode` VARCHAR(32) DEFAULT NULL,
  `pace_reason_summary` VARCHAR(255) DEFAULT NULL,
  `pace_trigger_source` VARCHAR(32) DEFAULT NULL,
